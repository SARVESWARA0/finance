...
import { 
  TrendingUp, DollarSign, BarChart3, Newspaper, Brain, Star, Activity, 
  ChevronDown, ChevronUp, ExternalLink, AlertTriangle, Target, Clock,
  TrendingDown, Shield, Zap, Award, Users, Globe, PieChart
} from 'lucide-react';
...
const ComprehensiveAnalysis = ({ structuredOutput, analysisResults }) => {
  ...
  const { 
    stock, company, summary, fundamentals, newsSentiment, 
    technicalAnalysis, riskAssessment, comparison, recommendation 
  } = structuredOutput;
      ...
      <div className="p-6 space-y-6">
          ...
          <div className="bg-gradient-to-r from-red-500/10 to-pink-500/10 border border-red-500/20 rounded-2xl p-6">
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-4 mb-4">
              <div className="bg-slate-800/40 rounded-xl p-4">
                ...
                <span className="text-white font-bold text-lg">{riskAssessment.volatility ? `${riskAssessment.volatility}%` : '—'}</span>
              </div>
              <div className="bg-slate-800/40 rounded-xl p-4">
              ...
              <div className="bg-slate-800/40 rounded-xl p-4">
                ...
                <span className="text-white font-bold text-lg">{riskAssessment.downsideRisk ? `${riskAssessment.downsideRisk}%` : '—'}</span>
              </div>
            </div>
        ...
        <div className="bg-gradient-to-r from-green-500/10 to-emerald-500/10 border border-green-500/20 rounded-2xl p-6">
          <div className="flex items-center justify-between mb-4">
            ...
            <div className={`px-3 py-1 rounded-full text-xs font-bold ${getSentimentColor(newsSentiment)} bg-green-500/20 border border-green-500/30`}>
              {newsSentiment.split(' - ')[0]?.toUpperCase() || 'NEUTRAL'}
            </div>
          </div>
          
          <p className="text-slate-300 leading-relaxed">{newsSentiment}</p>
        </div>
          ...
          <div className="bg-gradient-to-r from-indigo-500/10 to-purple-500/10 border border-indigo-500/20 rounded-2xl p-6">
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm uppercase tracking-wide text-indigo-400 font-bold">Competitor Comparison</span>
              </div>
              <button
                onClick={() => toggleSection('competitors')}
                className="text-indigo-400 hover:text-indigo-300 transition-colors"
              >
                {expandedSections.competitors ? <ChevronUp className="w-5 h-5" /> : <ChevronDown className="w-5 h-5" />}
              </button>
            </div>
                ...
                <table className="w-full text-sm">
                  ...
                  <tbody>
                    {comparison.competitors.map((comp, index) => (
                      <tr key={index} className="text-white hover:bg-slate-700/30 transition-colors border-b border-slate-700/30">
                        <td className="py-3 font-bold">{comp.symbol}</td>
                        <td className="text-right py-3">{comp.pe || '—'}</td>
                        <td className="text-right py-3">{comp.marketCap || '—'}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
        ...
        <div className="bg-gradient-to-r from-yellow-500/10 to-orange-500/10 border border-yellow-500/20 rounded-2xl p-6">
          <div className="flex items-center justify-between mb-4">
            ...
            >
              {expandedSections.recommendation ? <ChevronUp className="w-5 h-5" /> : <ChevronDown className="w-5 h-5" />}
            </button>
          </div>
