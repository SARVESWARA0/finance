              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {symbols.map((symbol) => {
                  ...
                  return (
                    <div
                      key={symbol}
                      className="group p-6 bg-gradient-to-r from-slate-800/50 to-slate-700/30 border border-slate-600/30 rounded-2xl hover:border-slate-500/50 transition-all duration-300 hover:scale-105"
                    >
                      <div className="flex items-center justify-between mb-4">
                        <div>
                          <h3 className="text-2xl font-bold text-white">{symbol}</h3>
                          {data.companyName && <p className="text-slate-400 text-sm mt-1">{data.companyName}</p>}
                        </div>
                        <div
                          className={`w-3 h-3 rounded-full ${isPositive ? "bg-green-400" : "bg-red-400"} animate-pulse`}
                        ></div>
                      </div>
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                ...
                <div className="p-4 bg-gradient-to-r from-blue-500/10 to-cyan-500/10 border border-blue-500/20 rounded-xl">
                  <div className="text-2xl font-bold text-blue-400 mb-1">
                    {
                      symbols.filter((s) => (bySymbol(s).priceChangePercentage || bySymbol(s).changePercent || 0) > 0)
                        .length
                    }
                  </div>
                ...
                <div className="p-4 bg-gradient-to-r from-purple-500/10 to-violet-500/10 border border-purple-500/20 rounded-xl">
                  <div className="text-2xl font-bold text-purple-400 mb-1">{Object.keys(news).length}</div>
            ...
            {symbols.length > 0 && Object.keys(indicators).length > 0 ? (
              symbols.map((symbol) => {
                ...
                return (
                  <div
                    key={symbol}
                    className="p-6 bg-gradient-to-r from-orange-500/10 to-red-500/10 border border-orange-500/20 rounded-2xl"
                  >
                    <div className="flex items-center gap-2 mb-4">
                      <Activity className="w-5 h-5 text-orange-400" />
                      <span className="text-xs uppercase tracking-wide text-orange-400 font-bold">
                        Technical Analysis
                      </span>
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                      ...
                      <div className="bg-slate-800/40 rounded-xl p-4">
                        ...
                        <span className="text-white font-bold text-lg">
                          {ind.latestClose !== undefined ? `$${ind.latestClose}` : "—"}
                        </span>
                      </div>
                    </div>
                  </div>
            ...
            {symbols.length > 0 && Object.keys(news).length > 0 ? (
              symbols.map((symbol) => {
                ...
                return (
                  <div
                  key={symbol}
                  className="p-6 bg-gradient-to-r from-blue-500/10 to-cyan-500/10 border border-blue-500/20 rounded-2xl"
                  >
                  <div className="flex items-center justify-between mb-6">
                    <div className="flex items-center gap-3">
                  ...
                  {newsData.headlines && newsData.headlines.length > 0 && (
                    ...
                    <div className="space-y-3">
                      {newsData.headlines
                      .slice(0, isExpanded ? newsData.headlines.length : 5)
                      .map((headline, i) => (
                        <a
                        key={i}
                        href={headline.link}
                    ...
                    <tbody>
                      {symbols.map((symbol) => {
                        ...
                        return (
                          <tr
                            key={symbol}
                            className="text-white hover:bg-slate-700/30 transition-colors border-b border-slate-700/30"
                          >
                            <td className="py-4 font-bold">
                              <div>
                                <div className="text-lg">{symbol}</div>
                                {data.companyName && <div className="text-xs text-slate-400">{data.companyName}</div>}
                              </div>
                            </td>
