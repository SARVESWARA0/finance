...
import { Search, X, ArrowRight, TrendingUp, BarChart3, Newspaper, Brain, AlertTriangle } from "lucide-react"
import EnhancedMarketInsights from "../app/components/enhanced-insights"
import ComprehensiveAnalysis from "../app/components/comprehensive-analysis"
  ...
  overrides: {
    h1: {
      component: ({ children, ...props }) => (
        <h1 className="text-2xl font-bold mb-6 text-white" {...props}>
          {children}
        </h1>
      ),
    },
    h2: {
      component: ({ children, ...props }) => (
        <h2 className="text-xl font-semibold mb-4 text-slate-200" {...props}>
          {children}
        </h2>
      ),
    },
    ...
    ul: {
      component: ({ children, ...props }) => (
        <ul className="list-disc pl-6 mb-4 text-slate-300" {...props}>
          {children}
        </ul>
      ),
    },
    ...
    th: {
      component: ({ children, ...props }) => (
        <th
          className="border border-slate-700 bg-slate-800 px-4 py-3 text-left font-semibold text-slate-200"
          {...props}
        >
          {children}
        </th>
    ...
    code: {
      component: ({ className, children, ...props }) => (
        <code
          className="bg-slate-800 border border-slate-700 rounded px-2 py-1 text-sm font-mono text-cyan-300"
          {...props}
        >
          {children}
        </code>
    ...
    pre: {
      component: ({ children, ...props }) => (
        <pre
          className="bg-slate-800 border border-slate-700 rounded-lg p-4 overflow-x-auto mb-6 font-mono text-sm text-slate-300"
          {...props}
        >
          {children}
        </pre>
  ...
  return (
    <div className="fixed inset-0 bg-gradient-to-br from-slate-950 via-slate-900 to-indigo-950 overflow-hidden">
      ...
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        ...
        <div
          className="absolute -bottom-40 -left-40 w-80 h-80 bg-gradient-to-r from-indigo-500/10 to-purple-500/10 rounded-full blur-3xl animate-pulse"
          style={{ animationDelay: "2s" }}
        ></div>
        <div
          className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-96 h-96 bg-gradient-to-r from-purple-500/5 to-pink-500/5 rounded-full blur-3xl animate-pulse"
        ...
        ></div>
        <div
          className="absolute top-3/4 right-1/4 w-1 h-1 bg-indigo-400/40 rounded-full animate-bounce"
          style={{ animationDelay: "3s" }}
        ></div>
        <div
          className="absolute bottom-1/4 left-3/4 w-1.5 h-1.5 bg-purple-400/30 rounded-full animate-bounce"
                ...
                <div className="mb-8">
                  <div className="text-center mb-6">
                    ...
                    <p className="text-slate-400 max-w-lg mx-auto text-lg leading-relaxed">
                      Get detailed insights including fundamentals, technical analysis, risk assessment, news sentiment,
